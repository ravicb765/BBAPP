...
The function returns a two-dimensional array. The first array represents selectors separated by
commas (eg. `sub1, sub2`), the second contains the relevant tokens for that selector. Possible token
types are:
...
The maintainers of `css-what` and thousands of other packages are working with Tidelift to deliver
