...
@implementation RCTSurface {
    ...
    RCTSurfaceView *_Nullable
    _view;
    RCTTouchHandler *_Nullable
    _touchHandler;
...
- (void)handleBridgeWillLoadJavaScriptNotification:(__unused NSNotification
...
- (BOOL)synchronouslyWaitForStage:(RCTSurfaceStage)stage timeout:(NSTimeInterval)timeout {
    ...
    auto timeoutOccurred = dispatch_semaphore_wait(semaphore, dispatch_time(DISPATCH_TIME_NOW,
                                                                            timeout *
                                                                            NSEC_PER_SEC));
...
- (void)rootShadowView:(__unused RCTRootShadowView

*)
rootShadowView didChangeIntrinsicSize
:(CGSize)intrinsicSize
{
    self.intrinsicSize = intrinsicSize;
}

- (void)rootShadowViewDidStartRendering:(__unused RCTSurfaceRootShadowView
...
- (void)rootShadowViewDidStartLayingOut:(__unused RCTSurfaceRootShadowView
...
- (BOOL)uiManager:(__unused RCTUIManager

*)
manager performMountingWithBlock
:(RCTUIManagerMountingBlock)block
{
    if (atomic_load(&_waitingForMountingStageOnMainQueue) &&
        (self.stage & RCTSurfaceStageSurfaceDidInitialLayout)) {
        // Atomic equivalent of `_waitingForMountingStageOnMainQueue = NO;`.
        atomic_fetch_and(&_waitingForMountingStageOnMainQueue, 0);
...
- (void)uiManagerDidPerformMounting:(__unused RCTUIManager
...
- (void)mountReactComponentWithBridge:(RCTBridge *)bridge
    ...
    [bridge enqueueJSCall:@"AppRegistry" method:@"runApplication" args:@[moduleName,
