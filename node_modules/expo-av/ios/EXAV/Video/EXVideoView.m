...
- (instancetype)initWithModuleRegistry:(nullable EXModuleRegistry
...
- (void)_removePlayer {
    if (_requestedFullscreenChangeRejecter) {
        _requestedFullscreenChangeRejecter(@"E_VIDEO_FULLSCREEN",
                                           @"Player is being removed, cancelling fullscreen change request.",
                                           nil);
        _requestedFullscreenChangeResolver = nil;
        _requestedFullscreenChangeRejecter = nil;
        _requestedFullscreenChange = NO;
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
    if ((object == _playerLayer || object == _playerViewController ||
         object == _fullscreenPlayerViewController) &&
        [keyPath isEqualToString:EXVideoReadyForDisplayKeyPath]) {
        if ([change objectForKey:NSKeyValueChangeNewKey] && _onReadyForDisplay) {
            ...
            if ([_data.player.currentItem.asset tracksWithMediaType:AVMediaTypeVideo].count > 0) {
                ...
                CGSize actualSize = CGSizeApplyAffineTransform(videoTrack.naturalSize,
                ...
                naturalSize = @{@"width": @(presentationSize.width),
                                ...
                                @"orientation": presentationSize.width < presentationSize.height
                                                ? @"portrait" : @"landscape"};
            }
        ...
        if (viewBounds.size.height != screen.size.height &&
            viewBounds.size.width != screen.size.width && _fullscreenPlayerPresented &&
            !_fullscreenPlayerViewController) {
        ...
        } else if (viewBounds.size.height == screen.size.height &&
                   viewBounds.size.width == screen.size.width && !_fullscreenPlayerPresented) {
...
- (void)setSource:(NSDictionary *)source
    ...
    _data = [[EXAVPlayerData alloc] initWithEXAV:_exAV
                             ...
                             withLoadFinishBlock:^(BOOL success, NSDictionary *successStatus,
                                                   NSString *error) {
                                 EX_ENSURE_STRONGIFY(self);
                                 if (success) {
...
- (void)setFullscreen:(BOOL)value
    ...
    if (!_data) {
        ...
        if (reject) {
            reject(@"E_VIDEO_FULLSCREEN", @"Fullscreen encountered an error: video is not loaded.",
                   nil);
        }
        return;
        ...
        if (_requestedFullscreenChangeRejecter) {
            _requestedFullscreenChangeRejecter(@"E_VIDEO_FULLSCREEN",
                                               @"Received newer request, cancelling fullscreen mode change request.",
                                               nil);
        }
        ...
        if (value && !_fullscreenPlayerPresented && !_fullscreenPlayerViewController) {
            ...
            reject(@"E_VIDEO_FULLSCREEN",
                   @"Fullscreen player is already being presented. Await the first change request.",
                   nil);
        } else if (!value && _fullscreenPlayerIsDismissing && _fullscreenPlayerViewController &&
                   reject) {
            // Fullscreen player should be dismissing, is already dismissing, but hasn't dismissed yet.
            reject(@"E_VIDEO_FULLSCREEN",
                   @"Fullscreen player is already being dismissed. Await the first change request.",
                   nil);
        } else if (!value && !_fullscreenPlayerPresented && _fullscreenPlayerViewController &&
                   reject) {
            // Fullscreen player is being presented and we receive request to dismiss it.
            reject(@"E_VIDEO_FULLSCREEN",
                   @"Fullscreen player is being presented. Await the `present` request and then dismiss the player.",
                   nil);
        } else if (value && _fullscreenPlayerIsDismissing && _fullscreenPlayerViewController &&
                   reject) {
            // Fullscreen player is being dismissed and we receive request to present it.
            reject(@"E_VIDEO_FULLSCREEN",
                   @"Fullscreen player is being dismissed. Await the `dismiss` request and then present the player again.",
                   nil);
        } else if (resolve) {
