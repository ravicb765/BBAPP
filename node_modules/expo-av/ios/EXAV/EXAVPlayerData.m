...
(^loadFinishBlock)(BOOL success, NSDictionary *successStatus, NSString *error);
...
NSMapTable<NSObject *, NSMutableSet<NSString *> *> *
observers;
...
- (instancetype)initWithEXAV:(EXAV *)exAV
         ...
         withLoadFinishBlock:(void (^)(BOOL success, NSDictionary *successStatus,
                                       NSString *error))loadFinishBlock {
    if ((self = [super init])) {
        _exAV = exAV;
...
- (void)warnIfNonExistingFile:(NSURL *)url {
    if ([url.scheme isEqualToString:@"file"]) {
        ...
        if (!fileExists || isDirectory) {
            EXLogWarn(
                    @"Expo-av: attempted to load an asset that doesn't exist: %@. This can happen when you persist an absolute path and then try to load from it. Make sure the provided path is relative to the application sandbox, e.g. `${FileSystem.documentDirectory}/some_file.m4a`.",
                    url.path);
        }
    }
}
...
- (void)_loadNewPlayer {
    ...
    if ([self _isBase64Audio:_url] &&
        [[NSProcessInfo processInfo] isOperatingSystemAtLeastVersion:(NSOperatingSystemVersion) {17,
                                                                                                 0,
                                                                                                 0}]) {
    ...
    AVURLAsset *avAsset = [AVURLAsset URLAssetWithURL:assetUrl options:@{
            AVURLAssetHTTPCookiesKey: cookies, @"AVURLAssetHTTPHeaderFieldsKey": _headers}];
    ...
    [avAsset loadValuesAsynchronouslyForKeys:@[@"duration"] completionHandler:^{
        ...
        self.player = [AVQueuePlayer queuePlayerWithItems:@[firstplayerItem, secondPlayerItem,
                                                            thirdPlayerItem]];
        if (self.player) {
...
- (BOOL)_hasZeroTolerance:(NSDictionary *)parameters {
    ...
    if (CMTimeCompare(
            CMTimeMakeWithSeconds(seekMillisToleranceBefore.doubleValue / 1000, NSEC_PER_SEC),
            kCMTimeZero) != 0) {
        return NO;
    }
    ...
    return CMTimeCompare(
            CMTimeMakeWithSeconds(seekMillisToleranceAfter.doubleValue / 1000, NSEC_PER_SEC),
            kCMTimeZero) == 0;
}
...
- (void)setStatus:(NSDictionary *)parameters
    ...
    if ([parameters objectForKey:EXAVPlayerDataStatusPositionMillisKeyPath] != nil) {
        ...
        mustSeek = [self _hasZeroTolerance:parameters] || currentPositionMillis.longValue !=
                                                          [self _getRoundedMillisFromCMTime:_currentPosition].longValue;
        if (mustSeek) {
            newPosition = CMTimeMakeWithSeconds(currentPositionMillis.doubleValue / 1000,
                                                NSEC_PER_SEC);
        }
    }
    ...
    if (mustSeek &&
        [parameters objectForKey:EXAVPlayerDataStatusSeekMillisToleranceBeforeKeyPath] != nil) {
        NSNumber *seekMillisToleranceBefore = parameters[EXAVPlayerDataStatusSeekMillisToleranceBeforeKeyPath];
        toleranceBefore = CMTimeMakeWithSeconds(seekMillisToleranceBefore.doubleValue / 1000,
                                                NSEC_PER_SEC);
    }

    // We need to set toleranceAfter only if we will seek
    if (mustSeek &&
        [parameters objectForKey:EXAVPlayerDataStatusSeekMillisToleranceAfterKeyPath] != nil) {
        NSNumber *seekMillisToleranceAfter = parameters[EXAVPlayerDataStatusSeekMillisToleranceAfterKeyPath];
        toleranceAfter = CMTimeMakeWithSeconds(seekMillisToleranceAfter.doubleValue / 1000,
                                               NSEC_PER_SEC);
    }
    ...
    if (_player && _isLoaded) {
        ...
        void (^applyPostSeekParameters)(BOOL) = ^(BOOL seekSucceeded) {
            ...
            if (audioSessionError) {
                if (reject) {
                    reject(@"E_AV_PLAY", @"Play encountered an error: audio session not activated.",
                           audioSessionError);
                }
        ...
        if (mustSeek) {
            [_player seekToTime:newPosition toleranceBefore:toleranceBefore toleranceAfter:toleranceAfter completionHandler:^(
                    BOOL seekSucceeded) {
...
- (NSNumber *)_getRoundedMillisFromCMTime:(CMTime)time {
    return CMTIME_IS_INVALID(time) || CMTIME_IS_INDEFINITE(time) ? nil : @((long) round(
            (CMTimeGetSeconds(time) * 1000)));
}
...
- (NSDictionary *)getStatus {
    ...
    if (_player.status == AVPlayerStatusReadyToPlay) {
        ...
        [currentItem.loadedTimeRanges enumerateObjectsUsingBlock:^(id obj, NSUInteger idx,
                                                                   BOOL *stop) {
        ...
        playableDurationMillis = [self _getRoundedMillisFromCMTime:CMTimeRangeGetEnd(
                effectiveTimeRange)];
    ...
    if (isPlaying) {
        ...
        isBuffering = !_player.currentItem.isPlaybackLikelyToKeepUp &&
                      _player.currentItem.isPlaybackBufferEmpty;
    }
    ...
    void (^didPlayToEndTimeObserverBlock)(NSNotification *note) = ^(NSNotification *note) {
        ...
        if (strongEXAV) {
            dispatch_async(strongEXAV.methodQueue, ^{
                EX_ENSURE_STRONGIFY(self);
                [self _callStatusUpdateCallbackWithExtraFields:@{
                        EXAVPlayerDataStatusDidJustFinishKeyPath: @(YES)}];
...
- (void)_updateTimeObserver {
    ...
    CMTime interval = CMTimeMakeWithSeconds(_progressUpdateIntervalMillis.doubleValue / 1000.0,
                                            NSEC_PER_SEC);
...
- (void)_addObserversForNewPlayer {
    ...
    NSUInteger currentItemObservationOptions =
        ...
        if (strongSelf) {
            if (object == strongSelf.player) {
                if ([keyPath isEqualToString:EXAVPlayerDataObserverStatusKeyPath]) {
                    switch (strongSelf.player.status) {
                        ...
                        case AVPlayerStatusReadyToPlay:
                            if (!strongSelf.isLoaded) {
                                if (strongSelf.player.currentItem.status ==
                                    AVPlayerItemStatusReadyToPlay) {
                                    [strongSelf _finishLoadingNewPlayer];
                                } else if (strongSelf.player.currentItem.status ==
                    ...
                    bool statusChanged =
                            strongSelf.player.timeControlStatus != strongSelf.timeControlStatus;
                    ...
                    if (strongSelf.timeControlStatus == AVPlayerTimeControlStatusPlaying &&
                        strongSelf.replayResolve) {
                        strongSelf.replayResolve([strongSelf getStatus]);
                        strongSelf.replayResolve = nil;
                    }
                ...
                if ([keyPath isEqualToString:EXAVPlayerDataObserverStatusKeyPath]) {
                    switch (strongSelf.player.currentItem.status) {
                        ...
                        case AVPlayerItemStatusReadyToPlay:
                            if (!strongSelf.isLoaded &&
                                strongSelf.player.status == AVPlayerItemStatusReadyToPlay) {
                                [strongSelf _finishLoadingNewPlayer];
                            }
                            break;
                    ...
                    [strongSelf _callStatusUpdateCallback];
                } else if ([keyPath isEqualToString:EXAVPlayerDataObserverMetadataKeyPath] &&
                           strongSelf.metadataUpdateCallback) {
                    NSArray < AVMetadataItem * >
                    *timedMetadata = strongSelf.player.currentItem.timedMetadata;
...
void EXTapFinalize(MTAudioProcessingTapRef tap) {
    AVAudioTapProcessorContext *context = (AVAudioTapProcessorContext *) MTAudioProcessingTapGetStorage(
            tap);
...
void EXTapPrepare(MTAudioProcessingTapRef tap, CMItemCount maxFrames,
...
void
EXTapProcess(MTAudioProcessingTapRef tap, CMItemCount numberFrames, MTAudioProcessingTapFlags flags,
...
- (NSString *)_fileExtensionForMimeType:(NSString *)mimeType {
    ...
    return (__bridge NSString *) (UTTypeCopyPreferredTagWithClass(uti,
...
- (NSURL *)_saveBase64UrlToTempFile:(NSURL *)url {
    ...
    NSInteger mimeTypeStartIndex =
            [[base64String componentsSeparatedByString:@":"] objectAtIndex:0].length + 1;
    NSInteger mimeTypeEndIndex = [[base64String componentsSeparatedByString:@";"] objectAtIndex:0].length;
    NSInteger dataStartIndex =
            [[base64String componentsSeparatedByString:@","] objectAtIndex:0].length + 1;
    NSString *mimeType = [base64String substringWithRange:NSMakeRange(mimeTypeStartIndex,
                                                                      mimeTypeEndIndex -
                                                                      mimeTypeStartIndex)];
