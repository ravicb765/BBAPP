...
NSMutableDictionary <NSNumber *, EXAVPlayerData *> *
soundDictionary;
@property(nonatomic, assign) BOOL isBeingObserved;
@property(nonatomic, strong)
NSHashTable <NSObject <EXAVObject> *> *
videoSet;
...
NSArray<Protocol *> *)exportedInterfaces
{
...
- (void)setModuleRegistry:(EXModuleRegistry *)expoModuleRegistry {
    ...
    [EXPermissionsMethodsDelegate                          registerRequesters:@[
            [EXAudioRecordingPermissionRequester new]] withPermissionsManager:_permissionsManager];
}
...
- (NSError *)_setAudioMode:(NSDictionary *)mode {
    ...
    if (!playsInSilentMode && interruptionMode == EXAudioInterruptionModeDuckOthers) {
        return EXErrorWithMessage(
                @"Impossible audio mode: playsInSilentMode == false and duckOthers == true cannot be set on iOS.");
    } else if (!playsInSilentMode && allowsRecording) {
        return EXErrorWithMessage(
                @"Impossible audio mode: playsInSilentMode == false and allowsRecording == true cannot be set on iOS.");
    } else if (!playsInSilentMode && shouldPlayInBackground) {
        return EXErrorWithMessage(
                @"Impossible audio mode: playsInSilentMode == false and staysActiveInBackground == true cannot be set on iOS.");
    } else {
        if (!allowsRecording) {
...
- (NSError *)_updateAudioSessionCategoryForAudioSessionMode:(EXAVAudioSessionMode)audioSessionMode {
    ...
    if (!_playsInSilentMode) {
        ...
        EXAudioInterruptionMode activeInterruptionMode =
                audioSessionMode == EXAVAudioSessionModeActiveMuted
                ? EXAudioInterruptionModeMixWithOthers : _audioInterruptionMode;
        NSString *category = _allowsAudioRecording ? AVAudioSessionCategoryPlayAndRecord
    ...
    if ([[_kernelAudioSessionManagerDelegate activeCategory] isEqual:requiredAudioCategory] &&
        [_kernelAudioSessionManagerDelegate activeCategoryOptions] ==
        requiredAudioCategoryOptions) {
        return nil;
    }
    ...
    if (_allowsAudioRecording) {
        ...
        requiredAudioCategoryOptions =
                requiredAudioCategoryOptions | AVAudioSessionCategoryOptionAllowBluetooth;
    }
...
- (EXAVAudioSessionMode)_getAudioSessionModeRequired {
    ...
    if (_audioRecorder) {
        ...
        } else if (_audioRecorderIsPreparing &&
                   audioSessionModeRequired == EXAVAudioSessionModeInactive) {
...
- (NSError *)promoteAudioSessionIfNecessary {
    if (!_audioIsEnabled) {
        return EXErrorWithMessage(
                @"Expo Audio is disabled, so the audio session could not be activated.");
    }
    if (_isBackgrounded && !_staysActiveInBackground &&
        ![_kernelAudioSessionManagerDelegate isActiveForModule:self]) {
        return EXErrorWithMessage(
...
- (void)_runBlock:(void (^)(EXAVPlayerData *data))block
  withSoundForKey:(nonnull NSNumber

*)
key
        withRejecter
:(EXPromiseRejectBlock)reject
{
    EXAVPlayerData *data = _soundDictionary[key];
    if (data) {
        block(data);
    } else {
        reject(@"E_AUDIO_NOPLAYER",
               @"Sound object not loaded. Did you unload it using Audio.unloadAsync?", nil);
    }
}
...
withEXVideoViewForTag:(nonnull NSNumber
...
:(EXPromiseRejectBlock)reject
{
    ...
    [[_expoModuleRegistry getModuleImplementingProtocol:@protocol(EXUIManager)] executeUIBlock:^(
            id view) {
        if ([view isKindOfClass:[EXVideoView class]]) {
            ...
            reject(@"E_VIDEO_TAGINCORRECT",
                   [NSString stringWithFormat:@"Invalid view returned from registry, expecting EXVideo, got: %@", view],
                   nil);
        }
...
<NSString *, NSString *> *)_getAVKeysForRecordingOptionsKeys:(NSString *)bitRateStrategy
{
...
- (void)_setNewAudioRecorderFilenameAndSettings:(NSDictionary *)optionsFromJS {
    ...
    NSDictionary < NSString * , NSString * >
                                *avKeysForRecordingOptionsKeys = [self _getAVKeysForRecordingOptionsKeys:bitRateStrategy];
...
- (NSDictionary *)_getAudioRecorderStatus {
    if (_audioRecorder) {
        ...
        int curDuration = [_audioRecorder isRecording] ? (curTimestamp -
                                                          _audioRecorderStartTimestamp) : 0;
...
- (BOOL)_checkAudioRecorderExistsOrReject:(EXPromiseRejectBlock)reject {
    if (_audioRecorder == nil) {
        reject(@"E_AUDIO_NORECORDER",
               @"Recorder does not exist. Prepare it first using Audio.prepareToRecordAsync.", nil);
    }
...
{
    return @[EXDidUpdatePlaybackStatusEventName, EXDidUpdateMetadataEventName,
             @"ExponentAV.onError"];
}
...
loadSuccess
        rejecter
:(EXPromiseRejectBlock)loadError)
{
    ...
    EXAVPlayerData *data = [[EXAVPlayerData alloc] initWithEXAV:self
                                                     withSource:source
                                                     withStatus:status
                                            withLoadFinishBlock:^(BOOL success,
                                                                  NSDictionary *successStatus,
                                                                  NSString *error) {
...
:(EXPromiseRejectBlock)reject)
{
    ...
    if (!_allowsAudioRecording) {
        reject(@"E_AUDIO_AUDIOMODE",
               @"Recording not allowed on iOS. Enable with Audio.setAudioModeAsync.", nil);
        return;
    }
    ...
    if (_audioRecorder && !error) {
        ...
        if (error) {
            ...
            reject(@"E_AUDIO_RECORDERNOTCREATED",
                   [NSString stringWithFormat:@"Prepare encountered an error: %@", error.description],
                   error);
            return;
            ...
            [self _removeAudioRecorder:YES];
            reject(@"E_AUDIO_RECORDERNOTCREATED",
                   @"Prepare encountered an error: recorder not prepared.", nil);
            return;
        }
        ...
        if (!_allowsAudioRecording) {
            ...
            if (!error) {
                if ([_audioRecorder record]) {
                    ...
                    reject(@"E_AUDIO_RECORDING",
                           @"Start encountered an error: recording not started.", nil);
                }
            } else {
                reject(@"E_AUDIO_RECORDING",
                       [NSString stringWithFormat:@"Start encountered an error: %@", error.description],
                       error);
            }
        } else {
            resolve([self _getAudioRecorderStatus]);
...
:(UMPromiseRejectBlock)reject)
{
    ...
    if (preferredInput != nil) {
        ...
        reject(@"E_AUDIO_SETINPUT_FAIL",
               [NSString stringWithFormat:@"Preferred input '%@' not found!", input], nil);
    }
}
