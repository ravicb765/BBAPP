...
rejects if any of the promises reject. The fulfilled value is the current iterable value or
`undefined` if `tester` never resolved to `true`.
...
This function will receive resolved values from `input` and is expected to return a
